...
def build_field_rejection_pdf(application: Application) -> bytes:
    if application.app_type != ApplicationType.ATTACHMENT:
        raise ValueError("Not an attachment application.")
    if application.status != ApplicationStatus.REJECTED:
        raise ValueError("Rejection letter is only available for rejected applications.")
    ...
    write_heading("Field training — rejection letter")
    write_line(f"Issued: {timezone.localdate().isoformat()}")
    write_line(f"Reference: {application.app_ref or str(application.id)}")
    y -= 6
    write_line(f"Student name: {stud.full_name}")
    ...
    y -= 12
    write_line(
        "We regret to inform you that your field attachment application has not been "
        "accepted by Hospital HR at this time."
    )
    reason = (application.hr_decision_reason or "").strip()
    if reason:
        y -= 6
        write_line(f"Reason: {reason}")
    y -= 12
    write_line(
        "You may contact your university office for guidance, or reapply after addressing "
        "the points raised. This letter was generated from the STUD placement system."
    )
    c.showPage()
    c.save()
    return buf.getvalue()
